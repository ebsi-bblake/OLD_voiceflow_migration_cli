...
const SUPPORTED_WANTED_TYPES: ReadonlySet<string> = new Set(["workspace.CRUD:REPLACE", "project.CRUD:REPLACE", "assistant.REPLACE", "workspace-folder.REPLACE"]);
...
type SyncCatalog = (auth: AuthContext, channel: string, wanted: readonly string[]) => Promise<readonly Row[]>;
export const syncCatalog: SyncCatalog = (auth, channel, wanted) => {
  if (!isSupportedRequest(wanted)) return Promise.reject(new OperationFault("INVALID_ARGUMENT"));
  return new Promise((resolve, reject) => {
    ...
    const settle = (): void => {
      ...
      else if (state.kind === "FAILED" || state.kind === "TIMED_OUT") reject(new OperationFault(state.code, state.retryable, state.diagnostic));
...
export const createProjectSecrets: CreateProjectSecrets = async (auth, assistantID, secrets) => {
  for (const secret of secrets) await createSecret(auth, assistantID, secret);
};
type ReconcileProjectSecrets = (auth: AuthContext, assistantID: string, versionID: string, secrets: readonly SecretEntry[]) => Promise<void>;
export const reconcileProjectSecrets: ReconcileProjectSecrets = async (auth, assistantID, versionID, secrets) => {
  const existing = await loadExistingSecrets(auth, versionID);
  for (const secret of secrets) await reconcileSecret(auth, assistantID, existing, secret);
};
const reconcileSecret = (auth: AuthContext, assistantID: string, existing: readonly ExistingSecret[], secret: SecretEntry): Promise<void> => {
  const match = existing.find((candidate) => candidate.name === secret.name);
  return match === undefined ? createSecret(auth, assistantID, secret) : updateSecret(auth, assistantID, match, secret);
};
