...
type CreateSecretDependencies = Readonly<{ readonly webSocket?: typeof WebSocket }>;
type CreateSecret = (
  auth: AuthContext,
  assistantID: string,
  secret: SecretEntry,
...
const trace = (event: string, fields: TraceFields = {}): void => debugLog("logux-secret", event, fields);
const traceFrame = (direction: "in" | "out", frame: LoguxFrame): void =>
  trace(`${direction} frame`, { frameType: frame[0], syncID: frame[1], ...summarizeLoguxAction(frame), ...summarizeSecretFailureFrame(frame) });
const randomActionNumber = (): number => Math.floor(Math.random() * 1_000_000_000) + 1;

/* oxlint-disable complexity -- protocol lifecycle branches are explicit. */
export const createSecret: CreateSecret = (auth, assistantID, secret, dependencies = {}) =>
  new Promise((resolve, reject) => {
    ...
    const settle = (error?: OperationFault): void => {
      ...
      const code = state.kind === "UNKNOWN_OUTCOME" && state.code === "DEPENDENCY_TIMEOUT" ? "DEPENDENCY_TIMEOUT" : "DEPENDENCY_FAILURE";
      reject(new OperationFault(code, true, `logux-${state.kind.toLowerCase()}-terminal`));
    };
      ...
      for (const effect of effects) {
        try {
          if (effect.kind === "send-mutation")
            send(["sync", effect.mutationSyncID, {
              type: "secret.CREATE_ONE_STARTED",
              payload: { context: { assistantID }, data: { name: secret.name, visibility: "masked", defaultValue: secret.value } },
    ...
    connection = startLoguxConnection({
      ...
      onEvent: (event) => {
        ...
        if (frame[0] === "error") {
          ...
          return settle(new OperationFault("DEPENDENCY_FAILURE", true, `logux-${state.kind.toLowerCase()}-error-frame`));
        }
        ...
        if (isSecretFailure(frame, actionID)) {
          dispatch({ kind: "error-frame" });
          return settle(new OperationFault("DEPENDENCY_FAILURE", true, `logux-${state.kind.toLowerCase()}-failed`));
        }
