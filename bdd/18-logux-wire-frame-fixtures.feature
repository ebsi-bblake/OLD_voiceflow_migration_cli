...
Feature: Use explicit sanitized Voiceflow Logux wire-frame fixtures
  ...
  Background:
    Given frame[0] is the Logux frame kind
    And frame[1] is the frame sync or error correlation value
  ...
  Scenario: Receive a correlated secret failure envelope
    ...
    And any server failure payload is treated as opaque
    And the matching failure becomes a dependency failure
    And the operation does not wait for a timeout
    And the raw error payload and secret value are not copied into diagnostics
