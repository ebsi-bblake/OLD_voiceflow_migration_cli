...
const logux = await import("../../xyops/voiceflow/logux/state-machine.ts");
...
defineStep("a complete migration event sequence is reduced", function () { let state = workflow.createMigrationWorkflow(identity()); const stages = [state.stage]; let terminalEffects = []; for (const event of [{ kind: "authentication-succeeded" }, { kind: "export-succeeded" }, { kind: "plan-succeeded", planID: "plan-1" }, { kind: "archive-preflight-result", collision: false }, { kind: "import-succeeded", importedProjectID: "imported" }, { kind: "secret-input-resolved" }, { kind: "secret-resolution-completed", empty: true }]) { const transition = workflow.transitionMigrationWorkflow(state, event); state = transition.state; terminalEffects = transition.effects; stages.push(state.stage); } this.value = { state, stages, terminalEffects }; });
...
defineStep("a completed workflow receives a late event", function () { let state = workflow.createMigrationWorkflow(identity()); for (const event of [{ kind: "authentication-succeeded" }, { kind: "export-succeeded" }, { kind: "plan-succeeded", planID: "plan-1" }, { kind: "archive-preflight-result", collision: false }, { kind: "import-succeeded", importedProjectID: "i" }, { kind: "secret-input-resolved" }, { kind: "secret-resolution-completed", empty: true }]) state = workflow.transitionMigrationWorkflow(state, event).state; this.value = workflow.transitionMigrationWorkflow(state, { kind: "import-succeeded", importedProjectID: "late" }); });
...
defineStep("stale and late events are sent to runtime reducers", function () { const state = workflow.createMigrationWorkflow(identity()); this.value = [workflow.transitionMigrationWorkflow(state, { kind: "import-succeeded", importedProjectID: "stale" }), workflow.transitionMigrationWorkflow({ ...state, stage: "COMPLETED" }, { kind: "import-succeeded", importedProjectID: "late" })]; });
...
defineStep("a migration dependency failure is reduced", function () { this.value = workflow.transitionMigrationWorkflow(workflow.createMigrationWorkflow(identity("operation-99")), { kind: "dependency-failure", code: "DEPENDENCY_FAILURE", diagnostic: "network" }).state; });
